...
# ---------- LLM helper ----------
def ask_llm(prompt, sys_prompt=None, temperature=0.7, max_tokens=2000):
...
def commands_list():
    ...
    cmds.extend([
        {"command": "/code ", "label": "Генератор кода"},
...
def handle_command():
    ...
    if cmd == "alias":
        ...
        if subcmd == "add":
            if len(args) < 3:
                return jsonify({"error": "Формат: /alias add <имя> plugin <имя_плагина> [аргументы через пробел] или /alias add <имя> llm <промпт>"})
            name = args[1]
            type_ = args[2].lower()
    ...
    # Встроенные команды
    if cmd == "help":
        return jsonify({"result": "Используйте меню Функции или пишите запросы. /alias для своих команд. /code для генерации кода."})
    elif cmd == "clear":
        contents.clear()
        ...
        if args:
            ...
            return jsonify({"result": "Промпт обновлен."})
        else:
        ...
        code_sys_prompt = "Ты опытный программист. Пиши только чистый код без лишних пояснений. Добавь комментарии на русском. Если просят конкретный язык — используй его. Выводи код в блоке ```язык ... ```"
        generated = ask_llm(f"Напиши код: {query}", sys_prompt=code_sys_prompt, temperature=0.3, max_tokens=3000)
        ...
        if "```" in generated:
            ...
            for i, p in enumerate(parts):
                if i > 0 and p.strip():
                    # Убираем название языка в начале
                    lines = p.strip().split("\n")
        ...
        if "javascript" in query.lower() or "js" in query.lower():
            ...
            ext = ".css"
        elif "rust" in query.lower() or "rs" in query.lower():
            ext = ".rs"
        elif "go" in query.lower() or "golang" in query.lower():
            ext = ".go"
...
def share_to_chat():
    ...
    return jsonify({
        "filename": filename,
        "code": code,
        "message": f"Файл `{filename}` готов к отправке в чат"
    })
