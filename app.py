...
# ---------- ОПРЕДЕЛЕНИЕ ЯЗЫКА ПРОГРАММИРОВАНИЯ (ИСПРАВЛЕНО) ----------
def detect_language(query):
    ...
    """
    q_lower = query.lower()
    # Убираем знаки препинания, оставляем только слова
    q_clean = re.sub(r'[^\w\s]', ' ', q_lower)
    words = set(q_clean.split())

    # Словарь: слово -> (расширение, название языка)
    lang_map = {
        # Python
        "python": (".py", "Python"),
        "пайтон": (".py", "Python"),
    ...
    # 2. Проверяем фразы с пробелами ("visual basic", "c++", etc.)
    for phrase, (ext, name) in lang_map.items():
        if " " in phrase and phrase in q_lower:
            return (ext, name)

    # 3. Проверяем паттерны "на языке X" или "в X"
    lang_patterns = [
        r'на\s+(?:языке\s+)?(\w+)',
        r'в\s+(\w+)',
        r'using\s+(\w+)',
...
# ---------- Генерация изображений (ДОБАВЛЕНО) ----------
...
def image_view():
    """Просмотр изображения в браузере (не скачивание)"""
    filename = request.args.get("file", "")
    if not filename:
        return jsonify({"error": "No filename"}), 400
